...
EMAIL_HOST_USER = os.getenv('EMAIL_HOST_USER') # roguesinister ваше имя пользователя,
# например если ваша почта user@yandex.ru, то сюда надо писать user, иными словами, это всё то что идёт до собаки
EMAIL_HOST_PASSWORD = os.getenv('EMAIL_HOST_PASSWORD') # cvbn4567zxcv пароль от почты
...
# DEFAULT_FROM_EMAIL = EMAIL_HOST_USER + 'roguesinister@yandex.ru' здесь указываем уже свою ПОЛНУЮ почту с которой будут отправляться письма